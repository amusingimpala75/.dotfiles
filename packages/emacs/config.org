...
(use-package eglot
  ...
  :hook
  ((c-ts-mode bash-ts-mode fennel-mode go-ts-mode haskell-mode nix-mode java-ts-mode js-ts-mode lua-ts-mode rustic-mode scala-mode) . eglot-ensure)
  :config
...
Font-locking in code blocks for export
#+begin_src emacs-lisp
...
(use-package ess
  ...
  (defun my/ess-eval-top-level-expression (&optional vis)
    ...
    (let ((current (treesit-node-at (point))))
      (while (not (string= "program" (treesit-node-type (treesit-node-parent current))))
        (setq current (treesit-node-parent current)))
      (ess-eval-region (treesit-node-start current) (treesit-node-end current) vis))))
...
(use-package dired
  ...
  (dired-mode . (lambda ()
                  ...
                  (unless (my/dired-sidebar-bufferp)
                    (setq-local dired-hide-details-preserved-columns '(1 3 5 6 7 8))
                    (dired-hide-details-mode)
                    (add-hook 'dired-after-readin-hook 'dired-sidebar-tui-dired-display nil t)))))
...
(use-package envrc
  ...
  (advice-add 'my/org-latex-export-to-docx :around #'envrc-propagate-environment))
#+end_src
