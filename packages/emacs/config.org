...
(use-package xt-mouse
  ...
  (add-hook 'after-make-frame-functions (lambda (frame)
                                          (xterm-mouse-mode t)))
  (add-hook 'delete-frame-functions (lambda (frame)
                                      (xterm-mouse-mode -1))))
#+end_src
...
(use-package org
  ...
  (defun my/count-words-buffer ()
    (interactive)
    (message "%d" (count-words (point-min) (point-max) nil))))
...
(use-package project
  :custom
  (project-mode-line t)
  (project-mode-line-format '(" [" (:eval (substring (project-mode-line-format) 1)) "]")))
#+end_src
